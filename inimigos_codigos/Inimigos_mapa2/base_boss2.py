    ...
    def carregar_animacoes(self):
        ...
        def carregar_frames(pasta, prefixo, inicio, fim, escala=4):
            ...
            for i in range(inicio, fim + 1):
                ...
                caminho = os.path.join('assets', 'inimigos', 'boss2', pasta, nome_arquivo)
                img = pygame.image.load(caminho).convert_alpha()
                img = pygame.transform.scale(img, (img.get_width() * escala, img.get_height() * escala))
                frames.append(img)
            return frames

        animacoes['parado'] = carregar_frames('parado', 'parado', 1, 8)
        animacoes['andando'] = carregar_frames('andando', 'andando', 1, 8)
        animacoes['morrendo'] = carregar_frames('morte', 'morte', 1, 4)
        animacoes['dano'] = carregar_frames('dano', 'dano', 1, 4)
        animacoes['ataque_fraco'] = carregar_frames('ataque1', 'ataque', 0, 8)
        animacoes['ataque_pesado'] = carregar_frames('ataque2', 'ataque', 0, 8)
        animacoes['ataque_especial'] = carregar_frames('ataque3', 'ataque', 1, 12)
